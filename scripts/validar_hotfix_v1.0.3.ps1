...
Write-Host '=== VALIDACION HYS VISION IA - HOTFIX v1.0.3 OPENVINO DYNAMIC OUTPUT SHAPES ===' -ForegroundColor Cyan
...
Write-Host 'Verificando input graph 640x640 y tolerancia a outputs dinamicos OpenVINO...' -ForegroundColor Cyan
$shapeCode = @'
...
def safe_shape(p):
    try:
        return tuple(int(x) for x in p.shape)
    except Exception:
        try:
            return str(p.partial_shape)
        except Exception:
            return '<dynamic>'
outs=[(_ov_port_name(p),safe_shape(p)) for p in d.output_ports]
...
if($LASTEXITCODE-ne0){throw '[FAIL] Inferencia OpenVINO 640x640 / outputs dinamicos'}
...
Write-Host '[PASS] HOTFIX v1.0.3 OPENVINO DYNAMIC OUTPUT SHAPES + VIDEO REAL VALIDADO' -ForegroundColor Green
