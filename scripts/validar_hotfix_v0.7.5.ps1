...
Write-Host '=== VALIDACION HYS VISION IA - HOTFIX QA v0.7.5 POWERSHELL SINGLETON ARRAYS ===' -ForegroundColor Cyan
...
# CRITICO EN WINDOWS POWERSHELL 5.1:
# Invoke-RestMethod + una lista JSON de un solo elemento puede convertirse en PSCustomObject escalar.
# Usamos @() EN EL PUNTO DE ASIGNACION para impedir que PowerShell desenvuelva el singleton.
$events = @(Invoke-RestMethod 'http://localhost:8160/api/v1/safety-events?limit=100' -Headers $headers)
$sample = $null
foreach ($item in $events) {
  if ($null -ne $item -and [int]$item.evidence_count -gt 0) { $sample=$item; break }
}
...
$url='http://localhost:8160/api/v1/safety-events/assignees'
if ($sampleId) { $url += "?event_id=$sampleId" }
$assignees = @(Invoke-RestMethod $url -Headers $headers)
Write-Host "[INFO] Responsables API count=$($assignees.Count)" -ForegroundColor DarkGray
if ($assignees.Count -gt 0) {
  foreach ($candidate in $assignees) { Write-Host "[INFO] candidato=$($candidate.email) id=$($candidate.id)" -ForegroundColor DarkGray }
}
...
if ($sampleId) {
  $evidence = @(Invoke-RestMethod "http://localhost:8160/api/v1/safety-events/$sampleId/evidence" -Headers $headers)
  $clip=$null
  foreach ($ev in $evidence) { if ($ev.kind -eq 'CLIP_PRE_EVENT') { $clip=$ev; break } }
  if ($clip) {
    $clipId=Assert-SingleGuid $clip.id 'clip.id'
    $tmp=Join-Path $env:TEMP ("hys-preview-v075-"+[guid]::NewGuid().ToString()+'.mp4')
    try {
      ...
      docker cp $tmp 'hysvision_backend:/tmp/hys-preview-v075.mp4' *> $null
      if ($LASTEXITCODE -ne 0) { throw '[FAIL] docker cp preview' }
      $probeLines=@(& docker compose exec -T backend ffprobe -v error -select_streams v:0 -show_entries stream=codec_name,codec_tag_string,pix_fmt -show_entries format=duration -of default=noprint_wrappers=1 /tmp/hys-preview-v075.mp4)
      $probeExit=$LASTEXITCODE; $probe=($probeLines -join "`n")
      & docker compose exec -T backend rm -f /tmp/hys-preview-v075.mp4 *> $null
  ...
  } else { Write-Host '[WARN] Evento QA sin CLIP_PRE_EVENT; runtime sintetico H.264 ya paso.' -ForegroundColor Yellow }
}
...
Write-Host '[PASS] HOTFIX QA v0.7.5 POWERSHELL SINGLETON ARRAYS VALIDADO' -ForegroundColor Green
