...
Write-Host '=== VALIDACION HYS VISION IA - HOTFIX v0.7.4 RESPONSABLES PRODUCCION ===' -ForegroundColor Cyan
...
$events = Normalize-Sequence (Invoke-RestMethod 'http://localhost:8160/api/v1/safety-events?limit=100' -Headers $headers)
$sample = $null
...
$assignees=Normalize-Sequence (Invoke-RestMethod $url -Headers $headers)
...
if ($sampleId) {
  $evidence=Normalize-Sequence (Invoke-RestMethod "http://localhost:8160/api/v1/safety-events/$sampleId/evidence" -Headers $headers)
  $clip=$null
  foreach ($ev in $evidence) { if ($ev.kind -eq 'CLIP_PRE_EVENT') { $clip=$ev; break } }
  if ($clip) {
    $clipId=Assert-SingleGuid $clip.id 'clip.id'
    $tmp=Join-Path $env:TEMP ("hys-preview-v074-"+[guid]::NewGuid().ToString()+'.mp4')
    try {
      ...
      docker cp $tmp 'hysvision_backend:/tmp/hys-preview-v074.mp4' *> $null
      if ($LASTEXITCODE -ne 0) { throw '[FAIL] docker cp preview' }
      $probeLines=@(& docker compose exec -T backend ffprobe -v error -select_streams v:0 -show_entries stream=codec_name,codec_tag_string,pix_fmt -show_entries format=duration -of default=noprint_wrappers=1 /tmp/hys-preview-v074.mp4)
      $probeExit=$LASTEXITCODE; $probe=($probeLines -join "`n")
      & docker compose exec -T backend rm -f /tmp/hys-preview-v074.mp4 *> $null
